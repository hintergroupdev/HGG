...
export default function LeadershipClient({ initialMembers = [] }) {
  const [members, setMembers] = useState(
    initialMembers && initialMembers.length > 0 ? initialMembers : defaultLeadershipMembers
  );
  ...
  const founder =
    sanitizedMembers.find(
      (m) =>
        m.employeeId === "HGG-001" ||
        m.id === "charles-n-hammond" ||
        m.id === "leader-charles-n-hammond" ||
        m.slug?.current === "charles-n-hammond" ||
        m.name?.toLowerCase().includes("hammond")
    ) || sanitizedMembers[0];
  const teamMembers = sanitizedMembers.filter((m) => m !== founder);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
            {filteredTeam.map((member, idx) => {
              ...
              const initials = memberName
                .replace(/^(Maj\.\s*Gen\.|Lt\.\s*Commander|Commander|Gen\.|Lt\.|Dr\.|Mr\.|Mrs\.|Ms\.)\s+/gi, "")
                .trim()
                .split(/\s+/)
                .map((n) => n[0])
                .slice(0, 2)
            ...
            {filteredTeam.length === 0 && (
              <div className="col-span-full py-12 text-center text-slate-500 bg-white rounded-2xl border border-slate-200/80 p-8 shadow-xs">
                <p className="text-sm font-medium text-slate-600">
                  Appointments to this leadership tier are currently undergoing institutional review and onboarding.
                </p>
              </div>
            )}
          </div>
