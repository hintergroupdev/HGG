const envUrl = process.env.NEXT_PUBLIC_SITE_URL?.trim();
export const SITE_URL =
  envUrl && envUrl.length > 0
    ? envUrl.replace(/\/+$/, '')
    : (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'https://hintergroupghana.com');

export const siteConfig = {
  name: 'THE HINTER GROUP GHANA LTD',
  shortName: 'HGG',
  legalName: 'THE HINTER GROUP GHANA LTD',
  tagline: 'Consulting + Ventures | Business Brokerage • Committed to Excellence',
  slogan: 'Connecting Opportunities. Building Partnerships. Facilitating Growth.',
  description:
    'THE HINTER GROUP GHANA LTD (HGG) is an authoritative strategic consulting, venture development, and business brokerage firm based in Accra, Ghana. We facilitate high-impact investments, infrastructure, clean energy transition, and public-private partnerships across Ghana, West Africa, and global corridors.',
  url: SITE_URL,
  ogImage: `${SITE_URL}/images/img_new_2.PNG`,
  logo: `${SITE_URL}/assets/logos/Logo/Logo_Horizontal_White.svg`,
  favicon: `${SITE_URL}/assets/logos/Favicon/Logo_Favicon.svg`,
  keywords: [
    'THE HINTER GROUP GHANA LTD',
    'HGG',
    'HGG LTD',
    'The Hinter Group',
    'Charles N. Hammond',
    'Ghana business consulting',
    'West Africa investment facilitation',
    'Ghana infrastructure development',
    'renewable energy transition Ghana',
    'business brokerage Africa',
    'business brokerage Ghana',
    'Ghana venture development',
    'public-private partnerships Ghana',
    'Accra corporate advisory',
    'sustainable resource recovery Ghana',
    'agribusiness value chain West Africa',
    'international trade facilitation Ghana',
    'Ghana corporate governance ESG',
    'Dzorwulu Accra consulting',
  ],
  contact: {
    email: 'info@hintergroupghana.com',
    phone: '+233 55 523 9544',
    phoneAlt: '+233 24 332 3339',
    phoneTertiary: '+233 24 426 5432',
    phones: [
      '+233 55 523 9544',
      '+233 24 332 3339',
      '+233 24 426 5432',
    ],
    address: '8 Teinor Street, Dzorwulu, Accra, GA-158-3464, Ghana',
    headOffice: '8 Teinor Street, Dzorwulu, Accra, GA-158-3464, Ghana',
    postalAddress: 'P.O. Box GP2951, Accra Central, Accra, Ghana',
    postalCode: 'GA-158-3464',
    locality: 'Dzorwulu, Accra',
    country: 'Ghana',
    website: 'https://www.hintergroupghana.com',
  },
  social: {
    linkedin: 'https://linkedin.com',
    twitter: 'https://x.com',
    facebook: 'https://facebook.com',
  },
};
